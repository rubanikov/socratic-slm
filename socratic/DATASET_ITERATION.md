# Dataset v1 → v2: what went wrong, and how the data fixed it

The project's core claim is that the dataset — not the training loop — decides the
outcome. This document is the evidence: the v1 dataset failed its own quality gate,
the failure was diagnosed from the gate's verdicts, and a v2 revision of the *data
generation process* (no model or training changes) resolved it.
...
The blueprint line is the control: everything *without* tutor replies sailed through.
The failure lived entirely in the tutor's own replies.

### The autopsy — one failure mode, precisely located
...
And per conversation, most tickets were small: 83 conversations had exactly 1 bad
...
| Final dataset | — | 1,000 train + 100 test + 120 eval_dev + 80 eval_final, all judge-verified (train_2000 extension in progress) |

Downstream — the proof the fix reached the model (probe-30, full judge):

| Category | Frontier pooled failure | Tuned s500 / s1000 |
|---|---|---|
| **how-to** (v1's failure epicenter, 30% of training data) | **68%** | **100%/100% both** |

The failure mode wasn't patched at training time; it was removed from the data, so
the model never learned it.

## 5. Residual and v3 direction

The worst category *inverted*: how-to became perfect; **emotional** is now weakest
(s500: 71.1%/60.0%) — the same smuggling mode resurfacing under emotional pressure,
and the category with the fewest clean examples (worst topic-dedup collisions,
highest ticket rate during generation). Doubling its data (75 → 150 examples, the
1000-rung) lifted it to 84.2%/80.0% (+13/+20) — confirming the lever. **v3 = oversample
emotional the way v2's mix oversampled how-to.**

## 6. The lesson

The quality gate and the eval must be the same code. Because they were, the MVP eval
didn't just score the dataset — it *located* the defect (which rule, which category,
which phrasing pattern, per turn), turned it into a generation-prompt patch and a
targeted repair queue, and then proved the fix end-to-end in the trained model's
category table. ~80% of the outcome was decided here; the fine-tune was a button-press.
